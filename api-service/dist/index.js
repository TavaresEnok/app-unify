...
app.post('/admin/auth/login', async (req, res) => {
    ...
    try {
        ...
        const authResponse = await axios_1.default.post(authUrl, {
            email,
            password,
...
app.post('/diagnostic/onu-signal', async (req, res) => {
    ...
    try {
        ...
        console.log('[ONU-Signal] Usando:', { sgpBaseUrl, token: (token === null || token === void 0 ? void 0 : token.substring(0, 10)) + '...', appName });
        const session = axios_1.default.create({
            httpsAgent: new https.Agent({ rejectUnauthorized: false }),
            headers: { 'User-Agent': 'Mozilla/5.0' }
        });
