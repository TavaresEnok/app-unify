...
import 'shared/widgets/troubleshooter_card.dart';
import 'shared/widgets/audit_result_card.dart'; // NOVO
...
class _DiagnosticoPageState extends State<DiagnosticoPage> {
    ...
    return StreamBuilder<DiagnosticoState>(
      ...
      builder: (context, snapshot) {
        ...
        final allDone = state.testResultsDisplay.entries.every((e) =>
            (e.value['status'] as TestStatus? ?? TestStatus.pending) ==
                TestStatus.success ||
            (e.value['status'] as TestStatus? ?? TestStatus.pending) ==
                TestStatus.error);
          ...
          body: Column(children: [
            ...
            Expanded(
              child: ListView(
                ...
                children: [
                  if (state.diagnosticResult != null) ...[
                    AuditResultCard(
                  ...
                  const SizedBox(height: 16),
                  _buildFiberInfoCard(context, state),
        ...
        if (status == TestStatus.pending || status == TestStatus.running)
          ...
          Align(
            ...
            child: TextButton.icon(
              ...
              label: const Text("Gerenciar Wi-Fi",
                  style: TextStyle(color: Colors.blueAccent)),
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => WifiManagementPage(service: _service),
                  ),
                );
              },
            ),
  ...
  Widget _buildTileStatusText(BuildContext context,
    ...
    switch (status) {
      case TestStatus.running:
        // FIX: Mostrar o resultado REAL se disponível, em vez de "Executando..." genérico
        text =
      ...
      case TestStatus.pending:
        ...
        break;
      case TestStatus.success:
        // Caso usem esse widget também para sucesso
        text = result ?? "Concluído";
        break;
    }
    return Padding(
      padding: const EdgeInsets.only(top: 16.0),
  ...
  Widget _buildFiberInfoCard(BuildContext context, DiagnosticoState state) {
    // Busca dados do SGP no estado (chave sgpONU)
    final status =
        state.testResultsDisplay['sgpONU']?['status'] as TestStatus? ??
            TestStatus.pending;
    final resultText = state.testResultsDisplay['sgpONU']?['result'] as String?;

    return DashboardCard(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _buildTileHeader(context, label: "Fibra Óptica (SGP)", status: status),
        if (status == TestStatus.pending || status == TestStatus.running)
          _buildTileStatusText(context, status: status, result: resultText)
            ...
            Expanded(
                child: Column(
                  ...
                  _buildDeviceInfoRow(context,
                      ...
                      title: "Sinal RX",
                      value: _parseResultLine(resultText, "Sinal RX:")),
                  const SizedBox(height: 16),
                  _buildDeviceInfoRow(context,
                      icon: Icons.router_outlined,
                      title: "Modelo ONU",
                      value: _parseResultLine(resultText, "Modelo:")),
                ])),
            const SizedBox(width: 16),
            Expanded(
                child: Column(
                  ...
                  _buildDeviceInfoRow(context,
                      ...
                      title: "Sinal TX",
                      value: _parseResultLine(resultText, "Sinal TX:")),
                  const SizedBox(height: 16),
                  _buildDeviceInfoRow(context,
                      icon: Icons.thermostat,
                      title: "Temperatura",
                      value: _parseResultLine(resultText, "Temperatura:")),
                ])),
          ]),
          if (resultText.contains("Status:")) ...[
            const SizedBox(height: 16),
            _buildDeviceInfoRow(context,
                icon: Icons.info,
                title: "Status da Conexão",
                value: _parseResultLine(resultText, "Status:")),
          ]
        ]
      ]),
    );
