...
import {onDocumentCreated} from "firebase-functions/v2/firestore"; // ADICIONADO
...
// ==========================================================
// Processamento de Requisições Assíncronas (Function Requests)
// ==========================================================
exports.processFunctionRequests = onDocumentCreated({region: "southamerica-east1", document: "function_requests/{requestId}"}, async (event) => {
    ...
    try {
        ...
        if (type === "CREATE_TICKET") {
             const {subject, message, providerId, providerName, userEmail, imageUrl, requesterUid} = payload;
             
             // Validação básica
             if (!providerId || !message) {
                 throw new Error("Dados incompletos para abrir chamado.");
             }

             // Criar o ticket na coleção 'chamados'
             const ticketRef = await db.collection("chamados").add({
                 assunto: subject || "Sem Assunto",
                 descricao: message,
                 providerId: providerId,
                 providerName: providerName || "Provedor",
                 clienteEmail: userEmail || "Anônimo",
                 imageUrl: imageUrl || null,
                 clienteUid: requesterUid,
                 status: "Aberto",
                 ...
                 mensagens: [
                     {
                         ...
                         anexoUrl: imageUrl || null
                     }
                 ]
             });
             
             result = { ticketId: ticketRef.id };
        } else {
            throw new Error(`Tipo de requisição desconhecido: ${type}`);
        }
