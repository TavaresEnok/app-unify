...
class _DiagnosticoPageState extends State<DiagnosticoPage>
  ...
  Future<void> _runDiagnostic() async {
    ...
    if (authService.usuario == null || configProvider.providerConfig == null) {
      _showError('Configuração não carregada');
      return;
    }
    ...
    try {
      ...
      final result = await _diagnosticService!.runFullDiagnostic(
        ...
        contractId: authService.usuario!.contratoId ?? 0,
        sgpParams: {
          'token': configProvider.providerConfig!.config.integrations.apiToken,
          'app':
              configProvider.providerConfig!.config.integrations.appName ??
              'app_diagnostico',
        },
        sgpBaseUrl:
  ...
  Widget build(BuildContext context) {
    ...
    final themeColor = providerConfig != null
        ? hexToColor(providerConfig.config.themeColor)
        : hexToColor('#673AB7');
