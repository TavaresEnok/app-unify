...
exports.handleSendPushNotificationRequest = (0, firestore_2.onDocumentCreated)({
    document: "function_requests/{requestId}",
    region: "southamerica-east1"
}, async (event) => {
    var _a, _b, _c, _d;
    const requestId = event.params.requestId;
    const requestData = (_a = event.data) === null || _a === void 0 ? void 0 : _a.data();
    if (!requestData || requestData.type !== 'SEND_PUSH_NOTIFICATION') {
        return null;
    }
    const responseRef = db.collection('function_responses').doc(requestId);
    const requesterUid = requestData.requesterUid;
    ...
    try {
        ...
        const user = await auth.getUser(requesterUid);
        const isSuperAdmin = ((_b = user.customClaims) === null || _b === void 0 ? void 0 : _b.superAdmin) === true;
        const userProviderId = (_c = user.customClaims) === null || _c === void 0 ? void 0 : _c.providerId;
        ...
        if (targetCpf) {
            ...
            if (clientDoc.exists) {
                const fcmToken = (_d = clientDoc.data()) === null || _d === void 0 ? void 0 : _d.fcmToken;
                if (fcmToken) {
                    try {
...
exports.handleGetProviderDashboardDataRequest = (0, firestore_2.onDocumentCreated)({
    ...
    const requestId = event.params.requestId;
    const requestData = (_a = event.data) === null || _a === void 0 ? void 0 : _a.data();
    if (!requestData || requestData.type !== 'GET_PROVIDER_DASHBOARD_DATA') {
        return null;
    }
    const responseRef = db.collection('function_responses').doc(requestId);
    const requesterUid = requestData.requesterUid;
...
exports.handleSgpApiProxyRequest = (0, firestore_2.onDocumentCreated)({
    ...
    const requestId = event.params.requestId;
    const requestData = (_a = event.data) === null || _a === void 0 ? void 0 : _a.data();
    if (!requestData || requestData.type !== 'SGP_API_PROXY') {
        return null;
    }
    const responseRef = db.collection('function_responses').doc(requestId);
    const requesterUid = requestData.requesterUid;
    ...
    try {
        if (!providerId)
            throw new Error("ProviderID obrigatório.");
        ...
        const config = {
            url: ((_b = pData.integrations) === null || _b === void 0 ? void 0 : _b.sgpBaseUrl) || ((_c = pData.details) === null || _c === void 0 ? void 0 : _c.systemUrl) || pData.sgpBaseUrl,
            token: ((_d = pData.integrations) === null || _d === void 0 ? void 0 : _d.apiToken) || ((_e = pData.details) === null || _e === void 0 ? void 0 : _e.apiToken) || pData.apiToken,
            app: ((_f = pData.integrations) === null || _f === void 0 ? void 0 : _f.appName) || ((_g = pData.details) === null || _g === void 0 ? void 0 : _g.appName) || pData.appName || 'APP-PROVEDOR'
        };
        ...
        if (action === 'sync') {
            ...
            const limit = (params === null || params === void 0 ? void 0 : params.limit) || 25;
            const offset = (params === null || params === void 0 ? void 0 : params.offset) || 0;
            const searchTerm = (params === null || params === void 0 ? void 0 : params.searchTerm) || '';
            let query = db.collection(`provedores/${providerId}/clientes`);
            if (searchTerm) {
...
exports.handleGetProviderTicketsRequest = (0, firestore_2.onDocumentCreated)({
    ...
    const requestId = event.params.requestId;
    const requestData = (_a = event.data) === null || _a === void 0 ? void 0 : _a.data();
    if (!requestData || requestData.type !== 'GET_PROVIDER_TICKETS') {
        return null;
    }
    const responseRef = db.collection('function_responses').doc(requestId);
    const requesterUid = requestData.requesterUid;
...
// ===========================================
exports.handleListAdminUsersRequest = (0, firestore_2.onDocumentCreated)({
    document: "function_requests/{requestId}",
    region: "southamerica-east1"
}, async (event) => {
    var _a, _b;
    const requestId = event.params.requestId;
    const requestData = (_a = event.data) === null || _a === void 0 ? void 0 : _a.data();
    if (!requestData || requestData.type !== 'LIST_ADMIN_USERS') {
        return null;
    }
    const responseRef = db.collection('function_responses').doc(requestId);
    const requesterUid = requestData.requesterUid;
    try {
        ...
        const users = listUsersResult.users.map(u => {
            ...
            return ({
                uid: u.uid,
                email: u.email,
                superAdmin: ((_a = u.customClaims) === null || _a === void 0 ? void 0 : _a.superAdmin) === true,
                providerId: ((_b = u.customClaims) === null || _b === void 0 ? void 0 : _b.providerId) || null
            });
        });
        await writeResponse(responseRef, {
            result: { users }
...
exports.handleDeleteAdminUserRequest = (0, firestore_2.onDocumentCreated)({
    document: "function_requests/{requestId}",
    region: "southamerica-east1"
}, async (event) => {
    var _a, _b, _c;
    const requestId = event.params.requestId;
    const requestData = (_a = event.data) === null || _a === void 0 ? void 0 : _a.data();
    if (!requestData || requestData.type !== 'DELETE_ADMIN_USER') {
        return null;
    }
    const responseRef = db.collection('function_responses').doc(requestId);
    const requesterUid = requestData.requesterUid;
    const targetUid = (_b = requestData.payload) === null || _b === void 0 ? void 0 : _b.uid;
    try {
        if (!requesterUid || !targetUid)
            throw new Error("Dados incompletos.");
        // Check Permissions
        const user = await auth.getUser(requesterUid);
        if (!((_c = user.customClaims) === null || _c === void 0 ? void 0 : _c.superAdmin))
            throw new Error("Permissão negada.");
        if (requesterUid === targetUid)
            throw new Error("Você não pode se apagar.");
