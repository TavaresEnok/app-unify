...
// ==========================================================
// IMPLEMENTAÇÃO DE CONSUMO COM HISTÓRICO
// ==========================================================
exports.getConsumptionData = onCall({region: "southamerica-east1"}, async (request: CallableRequest) => {
    // 1. Validation
    const { cpfCnpj, senha, sgpParams, sgpBaseUrl } = request.data;
    
    // Verificar se todos os parâmetros necessários estão presentes
    if (!cpfCnpj || !senha || !sgpParams || !sgpParams.token || !sgpParams.app || !sgpBaseUrl) {
         throw new HttpsError("invalid-argument", "Parâmetros obrigatórios faltando: cpfCnpj, senha, sgpParams (token, app), sgpBaseUrl");
    }

    try {
        // Configurar axios com agente HTTPS para ignorar certificados auto-assinados (comum em SGPs locais)
        const session = axios.create({
            httpsAgent: new https.Agent({ rejectUnauthorized: false }),
            headers: { 'User-Agent': 'Mozilla/5.0' }
        });

        // 2. Get Contract ID via 'consultacliente'
        // Isso autentica e recupera o ID interno do contrato necessário para o extrato
        const consultaResponse = await session.post(`${sgpBaseUrl}/ws/ura/consultacliente/`, {
        ...
        if (!consultaResponse.data || !Array.isArray(consultaResponse.data.contratos) || consultaResponse.data.contratos.length === 0) {
             throw new HttpsError("not-found", "Contrato não encontrado no SGP.");
        }
        
        const contratoId = consultaResponse.data.contratos[0].contratoId;

        // 3. Fetch History (Current + 5 previous months = 6 months total)
        const history = [];
        const today = new Date();
        
        // Helper function to fetch data for a specific date
        const fetchForDate = async (date: Date) => {
            const mes = date.getMonth() + 1; // getMonth() é 0-11
            const ano = date.getFullYear();
            
            const payload = new URLSearchParams();
            payload.append('cpfcnpj', cpfCnpj);
            payload.append('senha', senha);
            payload.append('contrato', contratoId.toString());
            payload.append('mes', mes.toString());
            payload.append('ano', ano.toString());

            try {
                const res = await session.post(`${sgpBaseUrl}/api/central/extratouso/`, payload);
                const d = res.data;
                
                // O SGP retorna 'total' em bytes.
                const totalBytes = d.total as number || 0;
                const usedGb = totalBytes / (1024 * 1024 * 1024);
                const planName = d.plano || "Plano não informado";
                
                return {
                    period: `${mes.toString().padStart(2, '0')}/${ano}`,
                    usedGb: usedGb,
                    planName: planName,
                    // raw: d // Opcional para debug
                };
            } catch (e) {
                logger.error(`Erro ao buscar consumo para ${mes}/${ano}:`, e);
                return null;
            }
        };

        // Fetch Current month
        const currentData = await fetchForDate(today);
        if (currentData) history.push(currentData);

        // Fetch Previous 5 months
        for (let i = 1; i <= 5; i++) {
            // Cria uma data 'i' meses atrás. O construtor de Date lida automaticamente com virada de ano.
        ...
        // Garantir que retornamos 'usedGb', 'planName', 'period' no nível superior para compatibilidade com o app antigo
        const current = history[0] || { usedGb: 0, planName: 'N/A', period: 'N/A' };

        return {
            usedGb: current.usedGb,
            planName: current.planName,
            period: current.period,
            history: history // Novo campo com o histórico
        };

    } catch (error: any) {
        logger.error("Erro em getConsumptionData:", error);
        throw new HttpsError("internal", error.message || "Erro interno ao buscar consumo.");
    }
});
